...
func TestBuildFidelityFields(t *testing.T) {
	req := &canonical.Request{
		MaxTokens:         128,
		MaxTokensField:    canonical.MaxTokensSourceCompletionTokens,
		Temperature:       ptrFloat(0.2),
		FrequencyPenalty:  ptrFloat(0.5),
		PresencePenalty:   ptrFloat(0.1),
		Seed:              ptrInt64(7),
		ParallelToolCalls: ptrBool(false),
		Thinking:          &canonical.ThinkingConfig{Effort: "medium", Mode: canonical.ThinkingEnabled},
	...
	body, err := BuildRequest(&canonical.Request{
		Messages: []canonical.Message{{
			...
			Content: []canonical.Block{
				...
				{Type: canonical.BlockAudio, Audio: &canonical.AudioSource{Kind: "base64", Data: "QUJD", Format: "wav"}},
