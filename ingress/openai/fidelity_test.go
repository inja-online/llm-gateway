...
func TestParseReasoningEffort(t *testing.T) {
	...
	if req.Thinking == nil || req.Thinking.Effort != "high" || req.Thinking.Mode != canonical.ThinkingEnabled {
...
func TestParseMaxTokensSource(t *testing.T) {
	...
	if req.MaxTokens != 10 || req.MaxTokensField != canonical.MaxTokensSourceTokens {
	...
	if req.MaxTokens != 42 || req.MaxTokensField != canonical.MaxTokensSourceCompletionTokens {
		t.Fatalf("%d %q", req.MaxTokens, req.MaxTokensField)
	}
	...
	if req.MaxTokens != 1 || req.MaxTokensField != canonical.MaxTokensSourceTokens {
...
func TestParseInputAudio(t *testing.T) {
	...
	a := blocks[1]
	if a.Type != canonical.BlockAudio || a.Audio == nil || a.Audio.Data != "QUJD" || a.Audio.Format != "wav" {
		t.Fatalf("%+v", a)
	}
