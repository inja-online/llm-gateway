...
func TestParseThinkingEnabledBudget(t *testing.T) {
	...
	if req.Thinking == nil || req.Thinking.Mode != canonical.ThinkingEnabled {
...
func TestParseThinkingAdaptiveAndDisabled(t *testing.T) {
	for _, tc := range []struct {
		raw  string
		mode canonical.ThinkingMode
	}{
		{`{"type":"adaptive"}`, canonical.ThinkingAdaptive},
		{`{"type":"disabled"}`, canonical.ThinkingDisabled},
	} {
		req, err := ParseRequest([]byte(`{
		...
		if req.Thinking == nil || req.Thinking.Mode != tc.mode {
