---
title: "Groq STT"
description: "Route speech-to-text to Groq while keeping chat on another provider via model prefixes and capabilities."
sidebar:
  order: 2
  label: "Groq STT"
  group:
...
- [`gateway.example.yaml`](https://github.com/inja-online/llm-gateway/blob/master/gateway.example.yaml) — `groq` block + `whisper-fast` alias comment
- [Compatibility matrix](/llm-gateway/reference/compatibility-matrix/) — Audio STT row + operator note
- [xAI provider guide](/llm-gateway/providers/xai/) — same `openai_compat` capability pattern for media
