...
description: "DashScope OpenAI-compatible mode with CN vs international base URLs and compatible-mode paths."
sidebar:
  order: 3
  label: "Qwen"
  group:
...
From [`gateway.example.yaml`](https://github.com/inja-online/llm-gateway/blob/master/gateway.example.yaml):
...
- [`gateway.example.yaml`](https://github.com/inja-online/llm-gateway/blob/master/gateway.example.yaml) — `qwen` / `qwen_intl`
- [Z.AI regions](/llm-gateway/providers/zai/) — same pattern for another dual-region host
