...
| OpenAI/Google → Anthropic + `caching.auto_breakpoints.enabled` | Opt-in inject `cache_control` on system/tools (client wins; see [cache-control-policy.md](/cache-control-policy/)) |
...
|---|---|---|
| OpenAI | `/v1/conversations*` (nested included) | **U** — HTTP **501** `not_implemented` (stateless; no gateway store). Prefer Responses + client state / Files. Decision: [conversations-decision.md](/conversations-decision/) (Option A). |
...
Official SDK wire coverage (headers, happy paths, no live keys): **[sdk-compatibility-matrix.md](/sdk-compatibility-matrix/)**.
...
- [Deprecation / field drops](/deprecation-policy/)
