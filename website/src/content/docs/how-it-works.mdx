...
See the [compatibility matrix](/llm-gateway/reference/compatibility-matrix/) for the full dialect × modality grid.
...
Details: [Prompt caching policy](/llm-gateway/guides/cache-control-policy/).
...
| Mode | Guide |
|------|-------|
| API key / `api_key_env` | [OAuth & upstream auth](/llm-gateway/guides/oauth-auth/) |
| `oauth2` client credentials / refresh | same |
| `client_bearer` multi-tenant user tokens | same |
| Service account / `token_file` / WIF | [WIF](/llm-gateway/guides/wif-identity/), [Vertex](/llm-gateway/guides/vertex-ai/) |
...
| WebSocket Realtime / Live | [Realtime guide](/llm-gateway/guides/realtime-websocket/) |
| Platform APIs | [Platform proxies](/llm-gateway/guides/platform-apis/) |
...
- [Getting started](/llm-gateway/start/getting-started/) — install and first request  
- [Claude Code checklist](/llm-gateway/guides/claude-code/) — point `ANTHROPIC_BASE_URL` at the gateway  
- [Compatibility matrix](/llm-gateway/reference/compatibility-matrix/) — what translates vs passthrough  
