...
**Gateway Media Contract v1** (Anthropic/Google-shaped media paths, when shipped) is versioned with the gateway: additive media fields → MINOR; breaking media field renames → MAJOR. See [docs/deprecation-policy.md](/llm-gateway/deprecation-policy.md/) for translation field drops.
...
- **Multipart/media security review:** [docs/security-multipart-review.md](/llm-gateway/security-multipart-review.md/) linked from SECURITY.md (size limits, filenames, SSRF URI pass-through, `key_hash` only).
...
- **Docs:** [SDK hermetic compatibility matrix](/llm-gateway/sdk-compatibility-matrix.md/) (OpenAI/Anthropic/Google; named hermetic tests; default CI has no `-tags live`).
...
- Docs: provider notes (OpenRouter, xAI, Z.AI regions, Qwen regions, Groq STT routing), [compatibility matrix](/llm-gateway/compatibility-matrix.md/), [deprecation policy](/llm-gateway/deprecation-policy.md/), [Claude Code checklist](/llm-gateway/claude-code-checklist.md/), CONTRIBUTING modality guide, this changelog.
