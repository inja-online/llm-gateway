---
title: "Claude Code with ChatGPT, Claude, SuperGrok & Gemini"
description: "Use ChatGPT Plus/Pro, Claude Pro/Max, SuperGrok, and Gemini/Antigravity consumer OAuth through Claude Code — login, PATH wrappers, live model lists."
sidebar:
  order: 0
...
| **SuperGrok** / X Premium+ | Grok CLI import or xAI device OAuth | Grok 4.5 / 4.6, Composer 2.5 |
| **Gemini** / Antigravity | Import `~/.gemini/jetski-standalone-oauth-token` (`auth login gemini`) | Gemini via Google dialect |

You can use **any combination**: GPT only, Grok only, Gemini only, GPT+Grok, Claude+GPT, all four, and so on.
...
- At least one subscription you own (ChatGPT, Claude, SuperGrok, and/or Gemini / Antigravity)
...
- **Google Gemini / Antigravity** consumer OAuth is for **personal accounts you own**. The Jetski token file has access/refresh tokens, **not** the OAuth client secret. Refresh needs `INJA_GATEWAY_GEMINI_CLIENT_SECRET` (see below). Never commit that secret.
...
llm-gateway auth login gemini     # aliases: google, antigravity — imports Jetski token
...
llm-gateway auth import gemini     # ~/.gemini/jetski-standalone-oauth-token (or GEMINI_AUTH_FILE)
```
...
The gateway loads tokens from the auth store and **refreshes** ChatGPT / Grok / Gemini access tokens before expiry (Gemini also needs the client secret — next section).
...
cc-gemini              # Gemini / Antigravity (alias: ccgm)
cc-gpt-grok            # GPT + Grok
cc-multi               # Claude + GPT + Grok
```

PATH wrappers (forwards every arg to `claude`; `--help` / `--version` skip the gateway):
...
claude-gemini               # Gemini — live model from GET /v1/models?live=1
claude-codex                # ChatGPT / Codex subscription
```

`claude-gemini` / `claude-codex` set `CC_SKIP_GATEWAY_UP=1` (they expect a running gateway) and pick a **live** upstream id when `CC_MODEL` is unset (`google/…` vs `chatgpt/…`). Fallback aliases: `gemini`, `gpt`.
...
| `llm-gateway auth login gemini` | Import Antigravity / Gemini CLI token (`google` / `antigravity` aliases) |
...
| `llm-gateway auth import gemini` | From `~/.gemini/jetski-standalone-oauth-token` (override: `GEMINI_AUTH_FILE`) |
...
### Gemini / Antigravity client secret (required to refresh)

`auth import gemini` copies **tokens** from `~/.gemini/jetski-standalone-oauth-token`. That file does **not** contain Google’s OAuth **client secret**. Access tokens expire in ~1 hour; refresh POSTs to `https://oauth2.googleapis.com/token` and Google rejects the grant without the secret.

Do **not** reuse a random `GOCSPX-…` from `gcloud` ADC or Cloud Code — those belong to a **different** OAuth client. The Gemini CLI / Antigravity (Jetski) client id is:

```text
1071006060591-tmhssin2h21lcre235vtolojh4g403ep.apps.googleusercontent.com
```

**Export the secret (personal machine only):**

1. Open [Google Cloud Console → APIs & Services → Credentials](https://console.cloud.google.com/apis/credentials).
2. Open the **OAuth 2.0 Client ID** whose client id matches the string above (Desktop / installed app used by Gemini CLI or Antigravity).
3. Reveal **Client secret** (`GOCSPX-…`). Copy it locally — never paste it into git, tickets, or chat.
4. Put it in your shell rc (mode `0600`) **or** let import write it into the 0600 store:

```bash
# Option A — env (gateway process must inherit this)
export INJA_GATEWAY_GEMINI_CLIENT_SECRET='GOCSPX-…'   # your secret

# Option B — persist into credentials.json on next import
export INJA_GATEWAY_GEMINI_CLIENT_SECRET='GOCSPX-…'
llm-gateway auth import gemini
unset INJA_GATEWAY_GEMINI_CLIENT_SECRET   # optional if store now has it

llm-gateway auth status   # gemini row should show a refresh token; secret is never printed
```

If you cannot find that client in Console, you do not own the OAuth app (Google’s first-party client). Then use a **Gemini API key** instead (`auth: api_key` / `x-goog-api-key`) — not consumer OAuth.

Never commit `GOCSPX-…` into YAML, examples, or this repo.
...
| `gemini` | Gemini / Antigravity only | `gemini-pro` | `gemini` | `gemini-flash` |
| `multi` | Claude + GPT + Grok | `opus` | `gpt` | `composer-2.5` |
...
| `CC_GEMINI_MID` | Gemini-only main model (default `gemini`) |
...
Those unknown ids also get **no** thinking / effort / ultracode unless advertised. Helpers inject:

- `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL_SUPPORTED_CAPABILITIES` and `ANTHROPIC_CUSTOM_MODEL_OPTION_SUPPORTED_CAPABILITIES` = `effort,xhigh_effort,thinking,adaptive_thinking,interleaved_thinking`
- session `--settings` with `alwaysThinkingEnabled`, `modelPicker.replaceBuiltInOptions: true`, and `modelPicker.options` using `behavesAs: "claude-fable-5"` for gateway aliases (`grok-4.6`, `gemini`, `sol`, `sonnet`, …)
- **no** default `--effort` / `CLAUDE_CODE_EFFORT_LEVEL` / `effortLevel` — those pin `/effort` until restart. Set `CLAUDE_CODE_EFFORT_LEVEL` yourself if you want a launch pin.

Ultracode is **session-scoped** — opt in with `CC_ULTRACODE=1`. It does not persist in `~/.claude/settings.json`. Path override: `CC_ULTRACODE_SETTINGS`. Your own `--settings` wins. `CLAUDE_CODE_DISABLE_WORKFLOWS` disables it.
...
| `gemini` / `gemini-flash` | `google/gemini-2.5-flash` (fallback; prefer live list) |
| `gemini-pro` | `google/gemini-2.5-pro` |

YAML aliases are **fallbacks**. Wrappers and `?live=1` pull current ids from each provider’s `/models` (Gemini names like `models/gemini-…` are stripped to `gemini-…`).

Sources: [Claude models](https://platform.claude.com/docs/en/about-claude/models/overview) · [GPT-5.6](https://openai.com/index/gpt-5-6/) · [xAI models](https://docs.x.ai/developers/models) · [Gemini models](https://ai.google.dev/gemini-api/docs/models).
...
| Claude Code → Gemini | **Translate** Anthropic Messages ↔ Google `generateContent` |
...
| `gemini` | Bearer access token (auto-refresh; needs client secret) | `https://generativelanguage.googleapis.com/v1beta` |
...
| `no credentials for chatgpt` (or claude/grok/gemini) | `llm-gateway auth login <provider>` or `auth import …` |
| `auth status` shows expired | Re-login; Grok/ChatGPT/Gemini refresh if a refresh token is present (Gemini also needs `INJA_GATEWAY_GEMINI_CLIENT_SECRET`) |
| Gemini refresh `invalid_client` / missing secret | Export the **matching** desktop client secret (see above). Do not use a `gcloud` ADC secret. |
| Gemini 401 on `generativelanguage.googleapis.com` | Confirm `auth status` gemini is not expired; restart gateway after adding the `google` provider to YAML |
...
| [`examples/scripts/claude-grok`](https://github.com/inja-online/llm-gateway/blob/master/examples/scripts/claude-grok) · [`claude-gemini`](https://github.com/inja-online/llm-gateway/blob/master/examples/scripts/claude-gemini) · [`claude-codex`](https://github.com/inja-online/llm-gateway/blob/master/examples/scripts/claude-codex) | PATH wrappers (`helpers install` → `scripts/`) |
