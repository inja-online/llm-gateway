...
- `--effort xhigh` (skip if you pass `--effort`)
- session `--settings` with `"ultracode": true` (xhigh + Workflow API), `alwaysThinkingEnabled`, and `modelPicker.options` using `behavesAs: "claude-fable-5"` for gateway aliases (`grok-4.6`, `gemini`, `sol`, `sonnet`, …)

Ultracode is **session-scoped** — it does not persist in `~/.claude/settings.json`. Path override: `CC_ULTRACODE_SETTINGS`. Your own `--settings` wins. `CLAUDE_CODE_DISABLE_WORKFLOWS` disables it.
