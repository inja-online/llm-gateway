...
Related: [Cursor + same subscriptions](/llm-gateway/guides/cursor-subscriptions/) · [Claude Code checklist](/llm-gateway/guides/claude-code/) · [OAuth & upstream auth](/llm-gateway/guides/oauth-auth/) · [xAI provider notes](/llm-gateway/providers/xai/)
...
- Built gateway binary: `go build -o llm-gateway ./cmd/gateway`
...
|---|---|---|---|---|
| `gpt` | ChatGPT only | `o3` | `gpt` | `gpt-mini` |
...
| `claude+gpt` | Claude + GPT | `opus` | `gpt` | `gpt-mini` |
...
cc-claude
cc-multi
cc-run gpt+grok   # arbitrary combo
cc-list
cc-gateway-env grok
cc-gateway-start  # runs gateway with subscription config
```
...
| Alias | Routes to (typical) |
|---|---|
| `sonnet` / `opus` / `haiku` / `claude` | `anthropic/…` Claude models |
| `gpt` / `gpt-mini` / `o3` | `chatgpt/…` ChatGPT/Codex models |
| `grok` / `grok-4.5` | `xai/grok-4.5` |
| `composer` / `composer-2.5` | `xai/composer-2.5` |

Edit aliases if your account exposes different ids (some plans use alternate names). List config-derived models:
...
| `cc-list: command not found: _inja_cc_list_profiles` | Re-`source examples/shell/claude-code-helpers.sh` (fixed for zsh) |
...
| [`examples/shell/claude-code-helpers.sh`](https://github.com/inja-online/llm-gateway/blob/master/examples/shell/claude-code-helpers.sh) | `cc-gpt` / `cc-grok` / `cc-run` … |
