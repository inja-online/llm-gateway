...
5. **Subscription headers** — for OAuth providers the gateway injects CLI-compatible headers (Claude: `oauth-2025-04-20` betas + `X-App: cli`; ChatGPT/Codex: `User-Agent` / `Originator` / `Chatgpt-Account-Id`).
6. **`GET /v1/models`** lists aliases/catalog ids only for providers you have logged into (credential-gated, offline).
7. **Usage** can be metered with `hooks.jsonl` (default `stdout` in the example config).

```
  claude CLI  ──ANTHROPIC_BASE_URL──►  :8787
       KEY (edge)                         │
                                          ├─ anthropic + Bearer + OAuth betas (claude creds)
                                          ├─ chatgpt + Bearer + Codex headers (chatgpt creds)
