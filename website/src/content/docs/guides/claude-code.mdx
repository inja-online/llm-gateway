---
title: "Claude Code"
description: "Point Claude Code at the gateway, route Anthropic-shaped traffic, and run the release checklist."
sidebar:
  order: 1
  label: "Claude Code"
  group:
...
Use this guide to run **Claude Code** through the gateway, and as a release checklist when Anthropic ingress/egress, streaming, headers, or routing change. Prefer hermetic Go tests over live Claude Code in CI; run the manual block when validating a release candidate.
...
| OpenAI-only fields | Dropped on translate (see [Deprecation policy](/llm-gateway/reference/deprecation-policy/)) |
