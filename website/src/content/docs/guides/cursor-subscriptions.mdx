...
Use **Cursor** with the same local gateway and **consumer subscriptions** as Claude Code: ChatGPT (Codex OAuth), Claude (setup-token), SuperGrok (Grok CLI / device OAuth), Gemini / Antigravity.
...
Related: [Any app](/llm-gateway/guides/app-integrations/) · [Claude Code](/llm-gateway/guides/claude-code-subscriptions/) · [Claude app](/llm-gateway/guides/claude-desktop-subscriptions/) · [Codex](/llm-gateway/guides/codex-subscriptions/) · [OAuth](/llm-gateway/guides/oauth-auth/) · [xAI](/llm-gateway/providers/xai/)
...
1. Gateway binary ([release](https://github.com/inja-online/llm-gateway/releases) or build from source).  
...
3. At least one subscription logged in:

```bash
llm-gateway auth login chatgpt
llm-gateway auth login claude     # optional
llm-gateway auth import grok      # or auth login grok
llm-gateway auth import gemini    # optional — Gemini / Antigravity
llm-gateway auth status
```

4. HTTPS gateway running (same as Claude Code):
...
### Automated model install (recommended)
