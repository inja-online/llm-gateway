...
Run **one** local gateway with your **ChatGPT / Claude / SuperGrok / Gemini** logins, then point **whatever client you like** at it. Same credentials, same models, two wire dialects.
...
Related: [Claude Code](/llm-gateway/guides/claude-code-subscriptions/) · [Claude app](/llm-gateway/guides/claude-desktop-subscriptions/) · [Codex](/llm-gateway/guides/codex-subscriptions/) · [Cursor](/llm-gateway/guides/cursor-subscriptions/) · [OAuth](/llm-gateway/guides/oauth-auth/)
...
llm-gateway auth import gemini     # Gemini / Antigravity Jetski token
...
**Not** session-only env: these write real config files. Fully quit Claude Desktop / Codex after switching. Continue, Cline, Aider, Windsurf, Cursor GUI are **not** auto-managed (print helpers only). T3 Code wrappers: `apps-t3-claude-wrappers` fills `customModels` on `claude-grok` / `claude-codex` / `claude-gemini` instances and hides Anthropic catalog rows.
...
**Full guide:** [Claude app + subscriptions](/llm-gateway/guides/claude-desktop-subscriptions/).
...
**Full guide:** [Codex + subscriptions](/llm-gateway/guides/codex-subscriptions/).
...
|-----|----------------|--------|
| **Claude Desktop** | [3P inference](/llm-gateway/guides/claude-desktop-subscriptions/) | Chat ignores `ANTHROPIC_BASE_URL`; picker needs `inferenceModels` for Grok |
...
| **Codex / ChatGPT coding** | [`~/.codex/config.toml`](/llm-gateway/guides/codex-subscriptions/) | Main ChatGPT **chat** UI may ignore custom base URL |
...
| Claude Desktop no change | Fully quit; Developer → Configure Third-Party Inference (see [Claude app guide](/llm-gateway/guides/claude-desktop-subscriptions/)) |
