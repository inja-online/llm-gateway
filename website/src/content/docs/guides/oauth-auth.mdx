...
**Subscription OAuth** (ChatGPT Codex, Claude setup-token, SuperGrok / Grok CLI, Gemini / Antigravity):

```bash
./llm-gateway auth login chatgpt
./llm-gateway auth login claude
./llm-gateway auth login grok      # or: auth import grok
./llm-gateway auth login gemini    # or: auth import gemini  (aliases: google, antigravity)
```

```yaml
auth: oauth2
oauth:
  credentials: chatgpt   # or claude | grok | gemini — tokens from local auth store
```

Gemini refresh needs `INJA_GATEWAY_GEMINI_CLIENT_SECRET` (Jetski token file has no secret). Export steps: [Claude Code + subscriptions](/llm-gateway/guides/claude-code-subscriptions/).

**Full walkthrough:** [Claude Code with ChatGPT, Claude, SuperGrok & Gemini](/llm-gateway/guides/claude-code-subscriptions/) — combos (`gpt`, `grok`, `gemini`, `gpt+grok`, `multi`), PATH wrappers `claude-grok` / `claude-gemini` / `claude-codex`, live `GET /v1/models?live=1`. Same gateway: [Claude app](/llm-gateway/guides/claude-desktop-subscriptions/) · [Codex](/llm-gateway/guides/codex-subscriptions/).

**ToS warning:** personal use of accounts you own only; do not resell multi-tenant consumer OAuth. Re-read OpenAI / Anthropic / xAI / Google terms.
