...
1. Point the tool `baseURL` at the gateway (e.g. `http://gateway:8787/v1`).  
2. Prefer **edge key + server `api_key_env` or `oauth2`**.  
3. If the tool already holds a provider access token, use `auth: client_bearer`.  

**ToS warning:** consumer ChatGPT / Claude.ai subscription OAuth is often restricted for multi-user products. Use operator-held credentials.
