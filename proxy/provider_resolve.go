...
// resolveAnthropicProvider picks a kind:anthropic provider when the request
// has no model field (Anthropic Files, future Batches).
//
// Precedence:
//  1. ?provider=NAME
//  2. X-Provider: NAME header
...
//
// Fail-closed: non-anthropic kinds are rejected.
