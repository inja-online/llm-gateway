...
# Detect install root (git checkout vs llm-gateway helpers install).
_inja_detect_root() {
  local d cand
  if [[ -n "${INJA_GATEWAY_ROOT:-}" ]]; then
    printf '%s' "$INJA_GATEWAY_ROOT"
    return
  fi
  # Parent of shell/: .../shell → install root or repo layouts
  d="$(cd "$_INJA_SHELL_DIR/.." && pwd)"
  # Install layout: ~/.config/inja-gateway/shell → ~/.config/inja-gateway
  if [[ -f "$d/claude-code-subscriptions.yaml" || -d "$d/certs" ]]; then
    printf '%s' "$d"
    return
  fi
  # Repo: examples/shell or cmd/gateway/shell
  for cand in \
    "$(cd "$_INJA_SHELL_DIR/../.." 2>/dev/null && pwd)" \
    "$(cd "$_INJA_SHELL_DIR/../../.." 2>/dev/null && pwd)"; do
    if [[ -f "$cand/go.mod" ]] && command grep -q 'inja-online/llm-gateway' "$cand/go.mod" 2>/dev/null; then
      printf '%s' "$cand"
      return
    fi
  done
  # Fallback: XDG config (helpers install destination)
  printf '%s' "${XDG_CONFIG_HOME:-$HOME/.config}/inja-gateway"
}

_INJA_REPO_ROOT="$(_inja_detect_root)"
...
_inja_cc_certs_dir() {
  ...
  # Install layout or repo examples/certs
  if [[ -d "$root/certs" ]]; then
    printf '%s' "$root/certs"
  elif [[ -d "$root/examples/certs" ]]; then
    printf '%s' "$root/examples/certs"
  else
    printf '%s/certs' "$(_inja_cc_state_dir)"
  fi
}
...
_inja_cc_default_config() {
  ...
  for f in \
    "$root/claude-code-subscriptions.yaml" \
    "$root/examples/configs/claude-code-subscriptions.yaml" \
    "$root/gateway.yaml" \
    "$HOME/.config/inja-gateway/claude-code-subscriptions.yaml"; do
    if [[ -f "$f" ]]; then
      printf '%s' "$f"
      return
    fi
